...
/** Abort outbound calls that hang, so a function never stalls to the platform timeout. */
...
export async function sbSelect(table, query = "") {
  ...
  const res = await fetch(restUrl(`${table}${query ? `?${query}` : ""}`), withTimeout({
    headers: baseHeaders(),
  }));
  if (!res.ok) {
...
export async function sbInsert(table, row, { onConflict } = {}) {
  ...
  const res = await fetch(restUrl(`${table}${q}`), withTimeout({
    method: "POST",
...
export async function sbUpdate(table, query, patch) {
  if (!hasSupabase()) return [];
  const res = await fetch(restUrl(`${table}?${query}`), withTimeout({
    method: "PATCH",
